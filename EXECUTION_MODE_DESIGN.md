...
工作分支：`feat/execution-mode-design`。方案提交后继续在此分支实装；2026-09-16 按用户要求合并至 custom-release。
...
维护负担：运行配置新增关键依赖时需更新指纹白名单；快捷编辑器只展示可表达的单节点配置；各语言模式名称与用途说明需同步。2026-09-16 按用户要求进行日常集成；远端 custom-release 仍为原基线，无冲突合并，并在合并结果上重新运行编排器单元测试。
