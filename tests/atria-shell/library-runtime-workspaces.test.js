...
describe('N9 Library / Runtime domain adapters', () => {
    ...
    test('normalizes Native Library and existing Runtime child routes without defining another router', () => {
        ...
        expect(LIBRARY_SECTIONS.map(section => section.id)).toEqual(['works', 'worlds-knowledge', 'prompt-presets', 'skills']);
        expect(normalizeLibrarySection({ child: { id: 'prompt-modules' } })).toBe('prompt-presets');
