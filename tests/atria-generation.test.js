// Characterization suite for src/endpoints/backends/atria-generation.js
...
//
// Notes on isolation:
//  - atria-generation.js keeps a module-level `generationJobs` Map and a
...
describe('appendGenerationEvent', () => {
    ...
    test('ignores frames wrapped in the atria envelope for text extraction', () => {
        const jobId = uniqueJobId('append-atria-env');
        const req = baseRequest({ body: { chat_completion_source: 'openai' } });
        const job = createGenerationJob(req, { job_id: jobId });
        appendGenerationEvent(job, JSON.stringify({
            atria: { generation_id: 'x', status: 'completed' },
        }));
        // Event recorded but text stays empty (atria envelope short-circuits extractor).
        expect(job.events).toHaveLength(1);
