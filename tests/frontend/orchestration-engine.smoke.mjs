...
try {
    ...
    const setup = async (page, recovering) => page.evaluate(async recovering => {
        ...
        panel.initRunPanel(); panel.openRunPanel();
        ...
        panelStore.finishRun({ runId: panelId, status: 'committed' }); panel.openRunPanel();
    ...
    await page.screenshot({ path: resolve(root, `../.git/engine-${channel}-mobile.png`) });
