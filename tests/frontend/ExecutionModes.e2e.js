...
// Run against an isolated, onboarded Atria server; no model credentials needed.
async function openModes(page) {
    await openExtensionsDrawer(page);
    await openInlineDrawer(page, 'orchestrator_settings');
    await expect(page.locator('#atri_orch_execution_mode')).toBeVisible();
}
...
test('legacy copy, explicit activation, simplified editing and reload preserve configuration', async ({ page, baseURL }) => {
    ...
    const mode = page.locator('#atri_orch_execution_mode');
    await mode.selectOption('single');
    await page.locator('button[data-atria-tabs-target="atri_orch_tabs"][data-atria-tab-key="agents"]').click();
    await page.locator('#atri_orch_single_agent_system_prompt').fill('Keep the letter secret.');
    await page.locator('#atri_orch_single_agent_user_prompt').fill('{{last_user}}');
    await page.locator('#atri_orch_copy_single').click();
    ...
    await expect(page.locator('#atri_orch_quick_system')).toHaveValue('Keep the letter secret.');
    ...
    await openModes(page);
    await expect(mode).toHaveValue('spec');
    await expect(page.locator('#atri_orch_quick_system')).toHaveValue('Check the letter before replying.');
    await mode.selectOption('single');
    await expect(page.locator('#atri_orch_single_agent_system_prompt')).toHaveValue('Keep the letter secret.');
});
...
test('output routing and compatible modes remain usable at mobile width', async ({ page, baseURL }, testInfo) => {
    ...
    await openModes(page);
    const mode = page.locator('#atri_orch_execution_mode');
    await page.locator('#atri_orch_output').selectOption('reply');
    await expect(mode).toHaveValue('director');
    await expect(page.locator('#atri_orch_mode_description')).toContainText('draft');
    await page.locator('#atri_orch_output').selectOption('guidance');
    await expect(mode).toHaveValue('loop');
    for (const value of ['spec', 'agenda', 'loop', 'single']) {
        await mode.selectOption(value);
        await expect(page.locator('#atri_orch_output')).toHaveValue('guidance');
    }
    await expect(page.locator('#atri_orch_mode_sources')).toBeVisible();
...
test('character copy stays active while the user explicitly edits a global preset', async ({ page, baseURL }) => {
    ...
    await openModes(page);
    await page.locator('#atri_orch_execution_mode').selectOption('spec');
    const globalName = `Global scope fixture ${Date.now()}`;
    await page.locator('#atri_orch_quick_template').click();
    ...
    await dialog.locator('[data-quick-activate]').check();
    await dialog.locator('.popup-button-ok').click();
    await expect(page.locator('#atri_orch_mode_sources')).toContainText('Effective: Character');
    const cardPrompt = await page.locator('#atri_orch_quick_system').inputValue();
    await page.locator('button[data-atria-tabs-target="atri_orch_tabs"][data-atria-tab-key="general"]').click();
    const selector = page.locator('#orchestrator_settings [data-atria-preset-select][data-mode="spec"]');
    ...
    await expect(page.locator('#atri_orch_mode_sources')).toContainText('Effective: Character');
    await expect(page.locator('#atri_orch_mode_sources')).toContainText('Editing: Global');
    await page.locator('#atri_orch_quick_system').fill('Global edit must not change the card.');
    await page.locator('#atri_orch_quick_system').blur();
    await expect.poll(() => page.evaluate(() => {
        const c = Atria.getContext();
