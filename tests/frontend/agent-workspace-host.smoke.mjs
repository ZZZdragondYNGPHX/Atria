...
try {
 ...
 console.log(JSON.stringify(await page.evaluate(()=>({presets:window.Atria.getContext().getCapabilityApi('orchestrator').listWorkspacePresets(),workspaceSettings:document.querySelector('#orchestrator_settings')?.textContent,errors:[]}))));
 const state=await page.evaluate(async()=>{const panel=await import('/scripts/agents/orchestrator/workspace/panel.js');panel.openWorkspace('Orchestration');return document.querySelector('#agent-memory-workspace').textContent;});
 ...
 for (const mode of ['spec','agenda','director']) {
   ...
   const count=await page.evaluate(mode=>window.Atria.getContext().capabilitySettings.orchestrator.agentWorkspace.presets.find(p=>p.mode===mode).planTemplate.nodes.length,mode);
   ...
   assert.equal(await page.evaluate(mode=>window.Atria.getContext().capabilitySettings.orchestrator.agentWorkspace.presets.find(p=>p.mode===mode).planTemplate.nodes.length,mode),count+1);
   ...
   assert.equal(await page.evaluate(mode=>window.Atria.getContext().capabilitySettings.orchestrator.agentWorkspace.presets.find(p=>p.mode===mode).planTemplate.nodes.length,mode),count);
 }
 ...
 const id=await page.evaluate(()=>window.Atria.getContext().capabilitySettings.orchestrator.agentWorkspace.bindings.defaultPresetId);
 await page.waitForTimeout(1800);await page.reload();
 await page.waitForFunction(()=>window.Atria?.getContext?.().getCapabilityApi?.('orchestrator')?.listWorkspacePresets);
 assert.equal(await page.evaluate(()=>window.Atria.getContext().capabilitySettings.orchestrator.agentWorkspace.bindings.defaultPresetId),id);
 ...
 await page.waitForFunction(value=>window.Atria.getContext().capabilitySettings.memory_graph.recallEnabled===value,!wasEnabled);
