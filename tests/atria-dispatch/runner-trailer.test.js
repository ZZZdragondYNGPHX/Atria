// Focused tests for the atria trailer frame the runner appends at the tail
...
// `{atria:{generation_id, persisted, status}}` envelope from the SSE stream
...
describe('runAtriaDispatch — trailer frame (Task 2C)', () => {
    ...
    // compose with the real atria-generation module used by the tests
    // above.
    ...
    test('trailer is ordered AFTER dispatch chunks in the on-wire event sequence', async () => {
        ...
        expect(wire[wire.length - 1].text).toContain('"atria":');
    });
});
