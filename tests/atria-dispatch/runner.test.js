...
describe('runAtriaDispatch', () => {
    test('missing x-atria-request-id header: server auto-mints and returns via x-atria-generation-id', async () => {
        const req = fakeRequest({ requestId: null });
        const res = fakeResponse();
        const select = () => async (ctx) => { ctx.emit.end(); };
        await runAtriaDispatch(req, res, { endpoint: 'test', select });
        expect(res.state.statusCode).toBe(200);
        const mintedId = res.state.headers['x-atria-generation-id'];
        expect(typeof mintedId).toBe('string');
        expect(mintedId.length).toBeGreaterThan(10);
    });

    test('body.atri_generation.job_id overrides header (client-supplied uuid path)', async () => {
        const { getTaskByRequestId } = await import('../../src/endpoints/backends/atria-generation.js');
        const req = fakeRequest({ requestId: 'header-id-should-lose', body: { atri_generation: { job_id: 'body-id-wins' } } });
        const res = fakeResponse();
        const select = () => async (ctx) => { ctx.emit.end(); };
        await runAtriaDispatch(req, res, { endpoint: 'test', select });
        expect(res.state.headers['x-atria-generation-id']).toBe('body-id-wins');
    ...
    test('200 immediately with x-atria headers, dispatch runs in background', async () => {
        ...
        await runAtriaDispatch(req, res, { endpoint: 'test', select });
        expect(res.state.statusCode).toBe(200);
        expect(res.state.headers['x-atria-generation-id']).toBe('ok-1');
    ...
    test('runner attaches job to request so getJobFromRequest works downstream', async () => {
        const { getJobFromRequest } = await import('../../src/endpoints/backends/atria-generation.js');
    ...
    test('runner emits legacy atria trailer SSE frame so client openai.js:4316 can read generation_id/persisted', async () => {
        ...
        const select = () => async (ctx) => {
            ...
            // append the {atria:{...}} frame AFTER dispatch has already
            // called emit.end.
        ...
        await runAtriaDispatch(req, res, { endpoint: 'test', select });
        await new Promise(r => setTimeout(r, 20));
        // Find the trailer frame with the atria envelope.
        const decoded = emittedChunks.map(b => new TextDecoder().decode(b)).join('');
        expect(decoded).toContain('"atria":');
