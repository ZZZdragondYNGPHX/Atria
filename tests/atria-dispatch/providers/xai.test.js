 ...
 */
function fakeCtx({ body = {}, onFetch, secret = 'xai-fake-key', signal } = {}) {
    const emitted = [];
...
describe('dispatchXai', () => {
    test('non-streaming: emits single chunk containing upstream JSON body then end', async () => {
        ...
        expect(String(url)).toContain('api.x.ai/v1/chat/completions');
        expect(init.method).toBe('POST');
        expect(init.headers['Authorization']).toBe('Bearer xai-fake-key');
        expect(init.signal).toBe(ctx.signal);
    });

    test('missing API key with no reverse_proxy/base_url: emits error, no fetch', async () => {
    ...
    test('ctx.signal aborted mid-request: fetch AbortError caught, emits error, no chunk', async () => {
        ...
        const dispatchPromise = dispatchXai(ctx);
