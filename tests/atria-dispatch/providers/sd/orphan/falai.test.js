...
import { dispatchSdFalai } from '../../../../../src/atria-dispatch/providers/sd/falai.js';
...
describe('dispatchSdFalai', () => {
    ...
    test('queue-submit HTTP 500 → head+chunk+end with raw error body (no emit.error)', async () => {
        const fetchMock = jest.fn(async () => new Response('falai blew up', { status: 500 }));
        const ctx = fakeCtx({ onFetch: fetchMock });
        await dispatchSdFalai(ctx);
        ...
        const chunks = ctx._emitted.filter(e => e.kind === 'chunk');
        expect(chunks).toHaveLength(1);
        expect(new TextDecoder().decode(chunks[0].data)).toBe('falai blew up');
