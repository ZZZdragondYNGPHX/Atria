...
import { dispatchSdChutes } from '../../../../../src/atria-dispatch/providers/sd/chutes.js';
...
describe('dispatchSdChutes', () => {
    ...
    test('upstream 500 → head+chunk+end with raw error body (no emit.error)', async () => {
        const ctx = fakeCtx({
            onFetch: jest.fn(async () => new Response('chutes blew up', { status: 500 })),
        });
        await dispatchSdChutes(ctx);
        ...
        const chunks = ctx._emitted.filter(e => e.kind === 'chunk');
        expect(chunks).toHaveLength(1);
        expect(new TextDecoder().decode(chunks[0].data)).toBe('chutes blew up');
