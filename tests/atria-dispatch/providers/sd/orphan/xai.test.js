...
import { dispatchSdXai } from '../../../../../src/atria-dispatch/providers/sd/xai.js';
...
describe('dispatchSdXai', () => {
    ...
    test('upstream 500 → head+chunk+end with raw error body (no emit.error)', async () => {
        const ctx = fakeCtx({
            onFetch: jest.fn(async () => new Response('xai blew up', { status: 500 })),
        });
        await dispatchSdXai(ctx);
        ...
        expect(new TextDecoder().decode(chunks[0].data)).toBe('xai blew up');
