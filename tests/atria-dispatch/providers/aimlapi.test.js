 ...
 */
function fakeCtx({ body = {}, onFetch, secret = 'aimlapi-fake-key', signal } = {}) {
    const emitted = [];
...
describe('dispatchAimlapi', () => {
    test('non-streaming: emits single chunk containing upstream JSON body then end', async () => {
        ...
        expect(String(url)).toContain('api.aimlapi.com/v1/chat/completions');
        ...
        expect(init.headers['X-Title']).toBe('Atria');
    ...
    test('ctx.signal aborted mid-request: fetch AbortError caught, emits error, no chunk', async () => {
        ...
        const dispatchPromise = dispatchAimlapi(ctx);
