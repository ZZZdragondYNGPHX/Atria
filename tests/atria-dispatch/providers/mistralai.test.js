 ...
 */
function fakeCtx({ body = {}, onFetch, secret = 'mistral-fake-key', signal } = {}) {
    const emitted = [];
...
describe('dispatchMistralAI', () => {
    test('non-streaming: emits single chunk containing upstream JSON body then end', async () => {
        ...
        expect(String(url)).toContain('api.mistral.ai/v1/chat/completions');
        expect(init.method).toBe('POST');
        expect(init.headers['Authorization']).toBe('Bearer mistral-fake-key');
        expect(init.signal).toBe(ctx.signal);
    });

    test('missing API key (no reverse_proxy): emits error, no fetch', async () => {
    ...
    test('ctx.signal aborted mid-request: fetch AbortError caught, emits error, no chunk', async () => {
        ...
        const dispatchPromise = dispatchMistralAI(ctx);
