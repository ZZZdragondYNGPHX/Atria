 ...
 * Mirrors the AI21 test harness (tests/atria-dispatch/providers/ai21.test.js)
 * so future providers reuse the same structural conventions.
 * onFetch: async (url, init) => Response.
 */
function fakeCtx({ body = {}, onFetch, secret = 'minimax-fake-key', signal } = {}) {
    const emitted = [];
...
describe('dispatchMinimax', () => {
    test('non-streaming: emits single chunk containing upstream JSON body then end; uses global endpoint by default', async () => {
        const ctx = fakeCtx();
        ...
        expect(init.headers['Authorization']).toBe('Bearer minimax-fake-key');
    ...
    test('streaming: forwards upstream SSE chunks verbatim then end; cn endpoint honored', async () => {
        const sseBody =
        ...
        const ctx = fakeCtx({
            body: { stream: true, minimax_endpoint: 'cn' },
    ...
    test('ctx.signal aborted mid-request: fetch AbortError caught, emits error, no chunk', async () => {
        ...
        const dispatchPromise = dispatchMinimax(ctx);
