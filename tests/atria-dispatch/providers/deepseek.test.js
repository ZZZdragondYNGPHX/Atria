 ...
 */
function fakeCtx({ body = {}, onFetch, secret = 'deepseek-fake-key', signal } = {}) {
    const emitted = [];
...
describe('dispatchDeepSeek', () => {
    test('non-streaming: emits single chunk containing upstream JSON body then end', async () => {
        ...
        expect(String(url)).toContain('api.deepseek.com/beta');
        expect(String(url)).toContain('/chat/completions');
        expect(init.method).toBe('POST');
        expect(init.headers['Authorization']).toBe('Bearer deepseek-fake-key');
        expect(init.signal).toBe(ctx.signal);
    });

    test('missing API key without proxy: emits error, no fetch', async () => {
    ...
    test('ctx.signal aborted mid-request: fetch AbortError caught, emits error, no chunk', async () => {
        ...
        const dispatchPromise = dispatchDeepSeek(ctx);
