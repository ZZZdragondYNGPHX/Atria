...
import { dispatchTextCompletions } from '../../../src/atria-dispatch/providers/text-completions/dispatch.js';
        ...
        test('TOGETHERAI pickBy → only TOGETHERAI_KEYS survive', async () => {
            const ctx = fakeCtx({
                body: {
                    ...
                    foobar_extra: 'nope',
                    atri_generation: { persist_target: 'x' },
                },
            });
            await dispatchTextCompletions(ctx);
            ...
            expect(parsed.atri_generation).toBeUndefined();
        });
