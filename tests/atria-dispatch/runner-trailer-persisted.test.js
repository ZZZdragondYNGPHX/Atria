...
// mixing mocked and non-mocked runs of atria-generation.js under one
...
// atria-generation.js. We hand-list only the symbols runner.js consumes.
...
jest.unstable_mockModule('../../src/endpoints/backends/atria-generation.js', () => ({
    ...
    attachJobToRequest: (request, job) => { request.atriaGenerationJob = job || null; },
    getJobFromRequest: (request) => request?.atriaGenerationJob || null,
    ...
    // full-featured implementation lives in atria-generation.js and does
...
const { runAtriaDispatch } = await import('../../src/atria-dispatch/runner.js');
...
describe('runAtriaDispatch — trailer reads job.persisted AFTER completeGenerationJobFromText', () => {
    test('persisted=true + status=completed round-trip into the trailer envelope', async () => {
        ...
        expect(payload.atria.persisted).toBe(true);
        expect(payload.atria.status).toBe('completed');
        expect(payload.atria.generation_id).toBe('trailer-persisted-1');
    });
});
