...
import { getTaskByRequestId } from '../../src/endpoints/backends/atria-generation.js';
...
describe('runAtriaDispatch — job.events envelope shape (WS delivery contract)', () => {
    ...
    test('emit.end lands as a `{kind:"end", data:null}` envelope entry', async () => {
        ...
        // then the runner appends its trailer chunk (`data: {"atria":{...}}\n\n`)
    ...
    test('ws-delivery eventToFrame produces non-null wire frames for every stored entry (integration guard against regression to string-payload shape)', async () => {
        ...
        const { runAtriaDispatch: run } = await import('../../src/atria-dispatch/runner.js');
