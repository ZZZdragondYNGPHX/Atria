...
import { dispatchClaude } from '../../src/atria-dispatch/providers/chat-completions/claude.js';
import { dispatchMakerSuite } from '../../src/atria-dispatch/providers/chat-completions/makersuite.js';
import { dispatchOpenAICompatible } from '../../src/atria-dispatch/providers/chat-completions/openai-compatible.js';
import { dispatchCohere } from '../../src/atria-dispatch/providers/chat-completions/cohere.js';
import { dispatchDeepSeek } from '../../src/atria-dispatch/providers/chat-completions/deepseek.js';
import { dispatchXai } from '../../src/atria-dispatch/providers/chat-completions/xai.js';
import { dispatchMistralAI } from '../../src/atria-dispatch/providers/chat-completions/mistralai.js';
import { dispatchAI21 } from '../../src/atria-dispatch/providers/chat-completions/ai21.js';
import { dispatchAimlapi } from '../../src/atria-dispatch/providers/chat-completions/aimlapi.js';
import { dispatchChutes } from '../../src/atria-dispatch/providers/chat-completions/chutes.js';
import { dispatchMinimax } from '../../src/atria-dispatch/providers/chat-completions/minimax.js';
import { dispatchElectronHub } from '../../src/atria-dispatch/providers/chat-completions/electronhub.js';
import { dispatchAzureOpenAI } from '../../src/atria-dispatch/providers/chat-completions/azure-openai.js';
import { dispatchOpenAIResponses } from '../../src/atria-dispatch/providers/chat-completions/openai-responses.js';

function fakeRequest({ requestId = null, body = {}, handle = 'alice' } = {}) {
    return {
        headers: requestId ? { 'x-atria-request-id': requestId } : {},
        body,
...
describe('/generate handler integration via runAtriaDispatch', () => {
    test('missing x-atria-request-id header: runner auto-mints and proceeds (200 with server-supplied id)', async () => {
        const req = fakeRequest({ requestId: null, body: { chat_completion_source: 'openai' } });
        const res = fakeResponse();
        await runAtriaDispatch(req, res, {
            endpoint: 'chat-completions',
            select: (body) => selectChatCompletionDispatch(body),
        });
        expect(res.state.statusCode).toBe(200);
        expect(typeof res.state.headers['x-atria-generation-id']).toBe('string');
        expect(res.state.headers['x-atria-generation-id'].length).toBeGreaterThan(10);
    });
    ...
    test('valid source → 200 + x-atria-generation-id header; task started; dispatch invoked in background', async () => {
        ...
        expect(res.state.statusCode).toBe(200);
        expect(res.state.headers['x-atria-generation-id']).toBe('ok-claude-1');
        expect(res.state.headers['x-atria-server-persisted']).toBe('0');
