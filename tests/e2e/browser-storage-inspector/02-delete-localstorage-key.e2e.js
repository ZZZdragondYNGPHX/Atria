...
test.describe('Browser Storage Inspector · delete localStorage key', () => {
    test('confirms delete, removes key, refreshes list', async ({ page }) => {
        ...
        // Click delete on atria-draft-x729
        const targetRow = inspector.locator('.storageInspectorEntry[data-key="atria-draft-x729"]');
        ...
        await expect(confirmDialog).toContainText('atria-draft-x729');  // label interpolation
        ...
        await expect(inspector.locator('.storageInspectorEntry[data-key="atria-draft-x729"]')).toHaveCount(0);

        // Verify actual localStorage: key gone, others still present
        const state = await page.evaluate(() => ({
            deleted: localStorage.getItem('atria-draft-x729'),
            keep1:   localStorage.getItem('atria-last-chat'),
            keep2:   localStorage.getItem('atria-theme'),
        }));
