...
//      writer real card exports use, so this is on-disk state Atria's
//      /api/characters loader parses natively).
//   2. Select the card via the visible list.
//   3. Assert ghost <optgroup data-atria-card-bound="1"> exists in
...
test.beforeAll(async () => {
    ...
    // `data.extensions.atria.chat_completion_preset =
...
test.describe('#40 — multi-slot card-bound presets: optgroup + default apply + manual swap', () => {
    test('card with two bound presets renders ghost optgroup and applies default, manual switch swaps body', async ({ page }) => {
        ...
        const optgroupState = await page.waitForFunction(() => {
            ...
            const options = Array.from(optgroup.querySelectorAll('option[data-atria-char-bound="1"]'));
        ...
        await page.evaluate(({ target }) => {
            ...
            const opt = $sel.find('option[data-atria-char-bound="1"]').filter((_i, el) => el.textContent === target).first();
        ...
        expect(selectedValue.startsWith('__atria_card__::')).toBe(true);
