...
//   1. A single <optgroup data-atria-card-bound="1"> hosts every card-bound
...
function renderCardBoundOptgroup(selectEl, character, presets) {
    selectEl.querySelectorAll('optgroup[data-atria-card-bound="1"]').forEach(n => n.remove());
...
test('renderCardBoundOptgroup creates one option per preset in optgroup', () => {
    ...
    const options = document.querySelectorAll('#settings_preset_openai optgroup[data-atria-card-bound="1"] option');
    expect(options).toHaveLength(2);
    expect(options[0].value.startsWith('__atria_card__::')).toBe(true);
    expect(options[0].textContent).toBe('Foo');
    expect(options[1].textContent).toBe('Bar');
    expect(options[0].getAttribute('data-atria-char-bound')).toBe('1');
});
