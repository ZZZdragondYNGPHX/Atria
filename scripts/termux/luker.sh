...
STATE_DIR="${LUKER_TERMUX_STATE_DIR:-${HOME}/.local/state/luker-termux}"
PID_FILE="${STATE_DIR}/server.pid"
LOG_FILE="${STATE_DIR}/server.log"
PORT="${LUKER_TERMUX_PORT:-8000}"
...
validate_port() {
  [[ "${PORT}" =~ ^[0-9]+$ ]] || fail "LUKER_TERMUX_PORT must be an integer."
  (( PORT >= 1 && PORT <= 65535 )) || fail "LUKER_TERMUX_PORT must be between 1 and 65535."
}
...
assert_install_ready() {
  ...
  [[ -f "${REPO_ROOT}/server.js" ]] || fail "Cannot find Luker server.js at ${REPO_ROOT}."
...
start_server() {
  ...
  if managed_process_alive; then
    if http_ready; then
      log "Luker is already running at ${URL}."
      open_browser
      return
    fi
    fail "A managed Luker process exists but is not ready. Run: luker-termux logs"
  fi

  rm -f "${PID_FILE}"
  if http_ready; then
    fail "Port ${PORT} is already serving HTTP but is not managed by luker-termux. Choose another port with LUKER_TERMUX_PORT."
  fi

  : > "${LOG_FILE}"
  cd "${REPO_ROOT}"

  if [[ "${LUKER_TERMUX_WAKE_LOCK:-0}" == "1" ]] && command -v termux-wake-lock >/dev/null 2>&1; then
  ...
  for attempt in $(seq 1 120); do
    if ! kill -0 "${pid}" 2>/dev/null; then
      rm -f "${PID_FILE}"
      log "Luker exited during startup. Last log lines:"
...
stop_server() {
  ...
  if ! pid="$(read_pid)"; then
    log "No managed Luker process is recorded."
    return
  fi
  ...
  log "Stopping Luker (PID ${pid})..."
  kill "${pid}" 2>/dev/null || true
  local i
  for i in $(seq 1 10); do
    if ! kill -0 "${pid}" 2>/dev/null; then
      rm -f "${PID_FILE}"
      if [[ "${LUKER_TERMUX_WAKE_LOCK:-0}" == "1" ]] && command -v termux-wake-unlock >/dev/null 2>&1; then
...
status_server() {
  ...
  if http_ready; then
    log "HTTP is reachable on ${URL}, but it is not managed by luker-termux."
    return 1
  fi
...
update_repo() {
  ...
  if (( was_running == 1 )); then
    ...
    log "Update complete. Start with: luker-termux start"
  fi
}

usage() {
  cat <<EOF2
Usage: luker-termux <command>

Commands:
  start        Start Luker in background and open ${URL}
  stop         Stop the managed Luker server
...
Environment:
  ...
  LUKER_TERMUX_STATE_DIR=~/.local/state/luker-termux
  LUKER_TERMUX_WAKE_LOCK=1   # optional; may increase battery use
EOF2
}
