...
当一份编排预设按名字把某个 agent 钉到某份提示词预设（`Sonnet · 慢节奏评审`、`Haiku · 快速` 等）时，这个名字是在生成时才解析的。查找顺序：

1. 当前角色的卡内嵌预设集合——只要卡上有一份内嵌预设的名字命中，直接用它。
2. 你本地的聊天补全预设库。
3. 前两者都没命中时，回退到编排器设置里钉住的全局运行时默认。

卡优先的顺序在角色卡携带自己推荐预设时很关键：编排 profile 和它引用的预设一起走，接收方无需单独导入匹配的预设。若接收方本地有同名预设，卡上的副本仍然在该角色范围内胜出——卡作者调好的提示词形态不会被接收方本地随便起的同名预设遮盖。

编排器抽屉里每个 agent 的预设选择器顶部，当加载了角色卡时会展示一个「本卡内嵌」optgroup，列出卡上的内嵌预设——所以你能在同一个 UI 里同时看到并选择卡上的和本地的预设。

## 把编排配置提交到角色卡时按需内嵌引用的预设

向当前角色卡提交编排配置时，Atria 会先预检 agent 引用的提示词预设名字，查出哪些还没内嵌到卡上。这里的「提交」既包括编排器抽屉里的「保存到角色卡覆写」，也包括「AI 迭代工作台」弹窗里的「应用到角色卡」——两种入口共用同一次预检。

若有未内嵌的引用，会弹出汇总窗口列出它们，提供三个动作：

- **全部内嵌**——把每个引用预设的本地内容复制到卡上，让编排 profile 在导出后端到端可跑。
- **只存名字**——按原样持久化编排 profile，不嵌入预设内容。接收方若没有同名本地预设，那些 agent 会走运行时默认回退。
- **取消**——中止提交。

在面板里编辑 agent 的预设引用本身不会触发弹窗——选择权属于提交这一步，不属于编辑那一步。

角色卡槽位如何管理、编辑、剥离连接字段，详见[角色卡绑定预设与人设](/zh-CN/improvements/card-bound-presets)。
