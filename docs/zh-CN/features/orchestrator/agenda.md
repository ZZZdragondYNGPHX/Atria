...
当前 Atria 随附固定预设 **Atri-agenda**。已有库会原位更新 `builtin-agenda`，保留默认、角色和会话绑定，不自动切换当前选择。缺失或被修改的内置定义会恢复为当前版本；需要自定义时请创建个人副本。其他预设 ID 不受影响。

模型和提示词通过 **Runtime Route** 解析。智能体配置使用原生路由引用，问号帮助打开 Runtime 的路由设置，不再导入旧聊天补全预设。

旧的插件专用、非 Director 和 Director 配套提示词 JSON 已移除。当前四模式内置提示词由原生编排预设提供，不依赖这些文件。此前手动导入的个人副本不会因此被删除。

编排按需使用状态整理、世界约束、角色推演、剧情推进、一致性审计和指导汇总六个工作节点。上限为 Planner 6 轮、并发 3、普通工作运行 10 次，全局限制仍可能进一步收紧派发。默认只读已注入的上下文，不启用额外业务工具；最终节点生成指导，由主聊天模型写正文。
