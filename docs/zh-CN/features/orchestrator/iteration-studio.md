...
- **程序驱动自动续轮。** 工作台在 AI 发出任意工具调用的回合里自动续到下一轮，在 AI 只回纯文本、不发工具的回合里停下来。**没有手动 Auto-Continue 开关，也没有 AI 侧的 continue / finalize 工具——工作台只看 AI 这轮有没有调工具。**
...
工作台 AI 读你当前的 profile，通过工具调用产出 patch。工作台只要 AI 这轮发了任何工具调用就会自动续到下一轮；一旦 AI 改回纯文本、不再调工具就停——AI 持续改，迭代就持续跑。

## 角色卡世界书冲突调和

在角色卡专属编排会话里打开工作台时，AI 还会顺便对照这张卡绑定的世界书。格式相关的世界书条目分两类，处理方式不一样：

- **过程强制类**——条目在指挥*模型运行过程中怎么思考*（强制思考模板、每轮必须打 CoT 前缀、"回答前先做 5W1H 检查"、"按 1 至 N 步依次执行后再回答"等）。这类条目对编排是毒——它会在每一轮工具调用都触发，把 agent 需要做规划和工具调用的通道挤掉。工作台会摘掉那条格式约束，把作者真正想要的认知意图（关心的话题、角度、人设习惯、场景锚点）作为叙事/人设/场景素材重写进去，让 agent 当作叙事输入读取，而不是当作新的输出规则。
- **最终输出形态类**——条目在描述*最终给用户看到的那一条回复长什么样*（"输出必须用 markdown"、"用标签包裹回复"、"末尾附上一段总结"、"说话用诗的格式"等）。这类是合理的风格偏好，工作台会保留——只是改写一下措辞，让"最终出口"的语义明确，避免编排过程中的规划节点、工具调用节点、复审节点也被这条形式约束绑住。

工作台首选只改写冲突那一句、保留条目的其它信息载荷；只有当整条几乎就是纯格式约束、没有可保留的内容时才直接禁用整条。任何情况都不会删除条目。

**审批流程。** 世界书调整都是*提案*，不会立即落盘。每条提案以一张 diff 卡片的形式出现在产生它的助手消息下方，带**批准** / **拒绝**按钮——和编排变更的逐条审查体验一致。只有获得批准的提案会在你点击 **Apply** 时落到本地世界书；被拒绝的提案直接丢弃；未做决定的留在面板里等你回来处理。输入框上方有一行汇总，显示待审批 / 已批准 / 已拒绝的计数；当只有世界书提案而没有编排变更等着落地时，这一行还会出现一个"提交已批准的世界书改动"按钮单独提交。
