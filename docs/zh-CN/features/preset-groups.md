...
预设分组是 Atria 为预设选择器引入的分组展示功能。当你积累了大量预设后，扁平的下拉列表会变得难以管理。预设分组允许你将预设归入命名分组，在下拉选择器中以分组 header + 成员的层级结构展示。

![预设分组](/images/presets/preset-groups.png)

上图展示了下拉里的实际效果：未分组的预设保持在顶层，下方依次列出几个分组 header；每个 header 自带成员数量标签和折叠 chevron，展开后成员以缩进形式显示，行尾的图标分别用于改组和删除预设。
...
Atria 实现了自定义的 Select2 适配器，用于在下拉选择器中渲染分组结构和操作按钮：

1. 收集所有真实 option 元素（跳过分组 header）
2. 保留自定义属性（如 `data-atria-char-bound`）
