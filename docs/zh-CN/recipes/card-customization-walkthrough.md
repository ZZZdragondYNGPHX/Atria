...
1. **查清运行时 ctx 表面。** 工作台先用 `atri_ctx_describe` 钻入运行时 ctx（必要时 `atri_docs_read` 翻阅相关文档），把工具要碰到的数据形状摸清楚——不靠猜。
2. **起草函数体并 dry-run。** 工作台用 `atri_orch_dry_run_custom_tool` 给函数体喂样本入参，在沙箱里跑一遍，让任何运行时异常在你被请求批准之前先暴露出来。
3. **落地校验通过的工具。** 工作台通过 `atri_orch_set_custom_tool` 挂卡；工具采用 read 模式（只读、无副作用），内部一段 JavaScript 读取本轮的草稿快照、扫描尾部标签。模式对应的启用开关会被自动切到 true，下一轮主代理就能看见这个工具。
4. **改主代理的系统提示词。** 工作台再调 `atri_orch_patch_director_main_agent_system_prompt`（或 loop / agenda 的对应工具）追加一条规则：生成的最后一步必须调用该工具，若结果显示缺标签则就地补全后再产出。
