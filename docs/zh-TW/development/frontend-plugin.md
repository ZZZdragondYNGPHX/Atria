...
| `CHAT_BRANCH_CREATED` | 聊天分支或檢查點建立 | `(payload)` — 見下文 |
...
```ts
{
  mesId: number,                  // 分支/檢查點訊息索引
  branchName: string,             // 新分支或檢查點聊天 ID / 檔名
...
`CHAT_BRANCH_CREATED` 在新分支或檢查點聊天檔案儲存後、UI 切換到新聊天之前觸發。如果你的外掛儲存了聊天綁定的狀態資料，應在此事件中將狀態複製或截斷到新的分支或檢查點。
