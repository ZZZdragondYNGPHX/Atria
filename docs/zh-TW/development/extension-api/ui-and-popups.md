...
| 欄位 | 型別 | 說明 |
|------|------|------|
| `id` | `string` | 分頁根元素的唯一 DOM id |
| `scope` | `string` | 面板 scope 鍵；不同 scope 各自保存獨立的持久化選擇 |
| `tabs` | `Array<{key,label,contentHtml}>` | 依顯示順序排列的分頁定義 |
| `defaultTab` | `string?` | 無持久化狀態時的初始分頁 key |
...
|------|------|------|
| `title` | `string` | 彈窗標題 |
| `bodyHtml` | `string` | 彈窗內文 HTML——傳入前請對不受信任的內容做轉義 |
