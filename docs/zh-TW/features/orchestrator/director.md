...
預設 director profile 出廠時配套 **24 個出廠 Skill**，agent 按需讀取。Skill 是相容 Anthropic 的緊湊知識包 —— 每一份裝載一條寫作規則、一份評審方法、或一份工作流契約，過去這些都內聯在系統提示詞裡。把它們移出 prompt、放進 `skill_read` 可存取的檔案裡，帶來三個效果：prompt 短而可編輯；同一條規則可被多個 agent 共用而不必拷貝；卡作者或預設作者可以自己分發變體。
...
模式級的幾條是每個預設 agent 共享的通用寫作規則。兩條主代理 Skill 裝著 7 步回合工作流和派遣協議。每條 per-sub-agent 方法 Skill 裝著該子代理的專屬契約 —— 例如 `event-summary-rules-zh` 是 `memory_curator` 完整的 V10 事件摘要寫作紀律。

預設 profile 相應預填了 `skills.visible`：模式級 visible 覆蓋那 5 條共享 Skill，主代理用 `["+", "director-turn-workflow-zh", "director-dispatch-protocol-zh"]`（繼承模式 + 追加兩條），每個子代理用 `["+", "<對應方法 Skill>"]`。所以開箱即用每個 agent 都拿到對的那摞疊，不需要手動繫結。

伺服器首次啟動時把 `data/<user>/skills/global/` 從 `default/skills/global/` 填充。之後，編排器面板的 **管理 Skills** 按鈕（以及 `import-bundled` API）是唯一覆蓋入口 —— 後續啟動不會有隱式自動更新。
...
預設主代理系統提示詞與預設的十二個子代理 id **強耦合**——它已經按「先派偵察、起草、再派評審、迭代修訂、最後落盤」的紀律調好了。要改的話推薦用 [AI 迭代工作台](/zh-TW/features/orchestrator/iteration-studio)用自然語言描述需求，讓它透過工具呼叫 patch 你的 profile。
:::
...
loop: "主代理坐在寫作台前" {
  ...
  consult: "找位顧問(預設 profile 自帶 12 個子代理)" {
    style.fill: "#fff3e0"
...
4. **預設 profile 自帶 12 個為 RP 最佳化過的子代理**:
   ...
   | `memory_scout` | 起草前的單源 scout，透過記憶圖唯讀 API 跑一次 LLM 級召回。列舉可見候選池，用 `memory_find_by_name` / `memory_keyword_search`（或設定了 embedding profile 時 `memory_vector_search`）定位命名實體或主題命中，必要時透過 `memory_expand_seeds` 鑽 rollup，然後回傳帶 signal level 的 ≤6 條引用列表（signal 來自 API 結構訊號：edge density、exposure、alwaysInject）。不讀 chat 或 lorebook（那是別的 scout 的活）。不改圖。 | 「`evt_42`（第 3 章外祖母線索）是 hub —— 其子節點裡有 draft 要回收的告白節拍。降權：`msg_18`（一次性提及，無後續）。」 |
   ...
   | `memory_curator` | 後置 mutation sub-agent，更新記憶圖，把本輪中會延續過場景的事實寫下來。多輪 observe-act 流程：查 schema，建立前用 `memory_find_by_name` 查重已有實體，用 `memory_node_edit` 打補丁欄位，用 `memory_link_upsert` / `memory_link_delete` 管理關係邊，Phase B 檢查 `memory_compaction_candidates` 並呼叫 `memory_compact_nodes` 把事件層級壓縮。**event 節點每次 dispatch 必出一個**（時間線連續性）;character_sheet / location_state 預設 SKIP，只在變化通過 24 小時持續性測試時才寫。 | 「建立 `evt_42`（Day 5 立誓節點）。編輯 `n_eileen` 加 `goal: '還債'`。新增 `n_eileen → debt_owed_to → n_protag` 邊。把 `evt_18,19,20` 壓縮成 `rollup_l1_06`。」 |
   ...
   預設主代理系統提示詞與這 12 個 id **強耦合**，按 id 指名排程，併為每個寫好了 task brief 的樣式。改子代理時，主代理提示詞也要同步改。
