...
CardApp 是 Atria 獨有的角色卡內嵌應用系統。它允許角色卡在 `data.extensions.card_app` 中定義小型應用（包含 HTML、JavaScript 和樣式），這些應用會在聊天介面中載入和渲染，為角色卡賦予互動式的動態能力。
...
2. **執行** — 應用透過 `ctx` 上下文物件與 Atria 互動，回應聊天事件並更新自身狀態
...
- **狀態追蹤** — 透過聊天變數（`ctx.getVariable` / `ctx.setVariable`）持久化好感度、任務進度、物品清單等資料；只有當資料是 CardApp 獨佔、不適合放進單個變數的結構化命名空間時，才用聊天狀態（`ctx.getChatState` / `ctx.updateChatState`）。
