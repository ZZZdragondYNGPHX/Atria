...
Atria 的設定檔為專案根目錄下的 `config.yaml`。首次啟動時，如果該檔案不存在，Atria 會從 `default/config.yaml` 複製一份預設設定。
...
SillyTavern 支援透過 HTTP、HTTPS 或 SOCKS 代理轉發出站請求（如 API 呼叫），Atria 繼承了這一功能：
...
啟用後，Atria 會提供一個 CORS 代理端點，用於前端跨域請求轉發。
...
啟用後，存取 Atria 需要輸入使用者名稱和密碼。適用於單一使用者場景。
...
::: tip
此命令需要在 Atria 專案根目錄下執行，且服務無需處於執行狀態。
:::
...
Atria 支援透過 Authelia 或 Authentik 等反向代理認證方案實現單點登入。詳細設定請參閱 [認證與配額](/zh-TW/improvements/auth-and-quota)。
...
設定 `whitelistMode: false` 後，Atria 要求至少存在另一種保護機制，否則啟動時會判定為不安全設定並直接終止行程（Docker 下表現為容器反覆重新啟動）。安全暴露服務時，請至少滿足以下任一條件：
...
Atria 支援四種使用者資料持久化後端，由 `mode` 選擇；只有對應的子區塊會被讀取。

- `fs`（預設）：每筆聊天/預設/世界書等都是 `<dataRoot>/<handle>/` 下的一個檔案。最適合單使用者安裝，也是最方便手動檢視的後端。
- `sqlite`：每位使用者一個獨立的 `atria-storage.sqlite` 檔案，位於 `<dataRoot>/<handle>/`。適合希望使用單檔案交易式儲存、又不想額外運行資料庫服務的安裝。
