...
function renderDefaultConflict(entry, index) {
    const div = document.createElement('div');
    div.className = 'atria-studio-merge-entry';
    div.dataset.entryIndex = String(index);

    const path = entry.edit.path || '(root)';
    const opName = entry.edit.op;
    div.innerHTML = `
        <div class="atria-studio-merge-head">
            <code class="atria-studio-merge-path">${escapeHtml(path)}</code>
            <span class="atria-studio-merge-op">${escapeHtml(opName)}</span>
            <span class="atria-studio-merge-reason">${escapeHtml(entry.reason)}</span>
        </div>
        <div class="atria-studio-merge-panes">
            <div class="atria-studio-merge-pane">
        ...
        <div class="atria-studio-merge-choices">
            ...
            <textarea class="atria-studio-merge-manual" rows="3" placeholder="Manual value (used only if Manual edit selected)"></textarea>
        </div>
    `;
    return div;
}
...
export async function showConflictResolution(conflicts, opts = {}) {
    ...
    container.className = 'atria-studio atria-studio-merge';

    const header = document.createElement('div');
    header.className = 'atria-studio-merge-header';
    ...
    conflicts.forEach((entry, idx) => {
        ...
        if (customHandler && typeof customHandler.renderConflict === 'function') {
            ...
            entryNode.className = 'atria-studio-merge-entry atria-studio-merge-entry-custom';
            entryNode.dataset.entryIndex = String(idx);
            entryNode.appendChild(customBody);
            const choices = document.createElement('div');
            choices.className = 'atria-studio-merge-choices';
            choices.innerHTML = `
    ...
    return conflicts.map((entry, idx) => {
        ...
        if (decision === 'manual') {
            const ta = container.querySelector(`.atria-studio-merge-entry[data-entry-index="${idx}"] .atria-studio-merge-manual`);
