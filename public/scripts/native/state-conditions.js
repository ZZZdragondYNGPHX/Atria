...
// Pure tri-state evaluator for Native snapshot state conditions.
// No provider reads, writes, template execution, network calls or model calls.

export const STATE_CONDITION_RESULT = Object.freeze({
    TRUE: 'true',
    FALSE: 'false',
    UNKNOWN: 'unknown',
});
...
export const STATE_CONDITION_OPERATORS = Object.freeze([
    'eq', 'neq', 'gt', 'gte', 'lt', 'lte', 'contains',
]);
const operators = new Set(STATE_CONDITION_OPERATORS);
...
function unknown(reason, condition, providerStatus = '') {
    return {
        status: STATE_CONDITION_RESULT.UNKNOWN,
        reason,
 ...
 */
export function evaluateStateCondition(condition, providers = []) {
    ...
    return {
        status: result ? STATE_CONDITION_RESULT.TRUE : STATE_CONDITION_RESULT.FALSE,
 ...
 */
export function evaluateStateConditions(conditions, providers = [], logic = 'all') {
    const list = Array.isArray(conditions) ? conditions.slice(0, MAX_CONDITIONS) : [];
    const normalizedLogic = logic === 'any' ? 'any' : 'all';
    if (list.length === 0) {
        return { status: STATE_CONDITION_RESULT.TRUE, logic: normalizedLogic, results: [], reason: 'no_conditions' };
    }

    const results = list.map(condition => evaluateStateCondition(condition, providers));
    const hasTrue = results.some(result => result.status === STATE_CONDITION_RESULT.TRUE);
    const hasFalse = results.some(result => result.status === STATE_CONDITION_RESULT.FALSE);
    const hasUnknown = results.some(result => result.status === STATE_CONDITION_RESULT.UNKNOWN);

    if (normalizedLogic === 'any') {
        if (hasTrue) return { status: STATE_CONDITION_RESULT.TRUE, logic: normalizedLogic, results, reason: 'matched' };
        if (hasUnknown) return { status: STATE_CONDITION_RESULT.UNKNOWN, logic: normalizedLogic, results, reason: 'unknown' };
        return { status: STATE_CONDITION_RESULT.FALSE, logic: normalizedLogic, results, reason: 'not_matched' };
    }

    if (hasFalse) return { status: STATE_CONDITION_RESULT.FALSE, logic: normalizedLogic, results, reason: 'not_matched' };
    if (hasUnknown) return { status: STATE_CONDITION_RESULT.UNKNOWN, logic: normalizedLogic, results, reason: 'unknown' };
    return { status: STATE_CONDITION_RESULT.TRUE, logic: normalizedLogic, results, reason: 'matched' };
}
