...
import { KNOWLEDGE_TARGET_KINDS, KNOWLEDGE_DELIVERY_POSITIONS, KNOWLEDGE_CONDITION_LOGIC, KNOWLEDGE_CONDITION_OPERATORS, validateKnowledgeEditorValue } from './knowledge-contracts.js';
...
export function mountKnowledgeEditor({ document: doc, root, value, label = 'Knowledge revision JSON', onReview, confirmDelete = confirmLibraryAction }) {
    ...
    function renderTargets(parent, entry) {
        ...
        targets.forEach((target, index) => {
            ...
            action(doc, row, 'Remove target rule', () => { targets.splice(index, 1); write(targets); changeView(); });
        });
        action(doc, parent, 'Add target rule', () => { targets.push('narrator'); write(targets); changeView(); }, { disabled: targets.length >= 32 });
