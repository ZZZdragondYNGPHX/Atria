...
import { fields, id, json, text, valueTemplate, expression } from './v2-values.js';
...
export function compileUiDocument(raw, { mode } = {}) {
    raw = json(raw);
    fields(raw, ['schemaVersion', 'stateVersion', 'localState', 'preferences', 'selectors', 'actions', 'views', 'opening'], 'UI Document');
    ...
    const localState = compileState(raw.localState);
    ...
    for (const [key, source] of Object.entries(raw.selectors ?? {})) selectors[id(key)] = expression(source);
    const actions = {};
    ...
    for (const [key, action] of Object.entries(raw.actions ?? {})) {
        ...
        const steps = action.steps.map(step => {
            ...
            return Object.freeze({ ...step, when: step.when === undefined ? null : expression(step.when),
                value: step.value === undefined ? null : valueTemplate(step.value), args: valueTemplate(step.args ?? {}) });
        });
        ...
        const constraints = (action.constraints ?? []).map(rule => {
            ...
            return { ...rule, when: expression(rule.when), reasonCode: text(rule.reasonCode, 128), playerMessage: text(rule.playerMessage, 1024) };
        });
    ...
    function node(rawNode, depth = 0, inRepeat = false, inForm = false) {
        ...
        if (rawNode.type === 'native-slot' && (mode === 'component' || inRepeat)) throw new Error('Native slot ownership is invalid');
        ...
        for (const [key, value] of Object.entries(rawNode.bindings ?? {})) bindings[key] = valueTemplate(value);
        ...
        return Object.freeze({ ...rawNode, props, bindings, events, children: children.map(child => node(child, depth + 1, inRepeat || repeat, inForm || rawNode.type === 'form')),
            ...(repeat ? { source: expression(rawNode.source), key: expression(rawNode.key), emptyText: text(rawNode.emptyText ?? '') } : {}) });
    }
    ...
    if (raw.opening !== undefined) {
        ...
        const steps = raw.opening.steps.map(step => {
            ...
            return { ...step, next: step.next.map(edge => { fields(edge, ['when', 'to'], 'Opening transition'); return { to: id(edge.to), when: expression(edge.when) }; }) };
        });
    ...
    return Object.freeze({ schemaVersion: 2, stateVersion: raw.stateVersion, localState, preferences, selectors, actions, views, opening });
}
