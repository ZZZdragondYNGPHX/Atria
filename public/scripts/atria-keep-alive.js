...
// treats this tab as foreground media.
let audioCtx = null;
...
function hasAndroidKeepAliveBridge() {
    return typeof window !== 'undefined'
        ...
        && typeof window.AtriaAndroid.setBackgroundKeepAliveEnabled === 'function';
}
...
async function enterAudio() {
    ...
    if ('mediaSession' in navigator) {
        try {
            ...
            console.warn('[Atria] MediaSession setup failed', error);
        }
    }
}
...
export async function setKeepAliveMode(desired) {
    ...
    if (activeMode === 'android' && target !== 'android') {
        try {
            window.AtriaAndroid.setBackgroundKeepAliveEnabled(false);
        } catch (error) {
            console.warn('[Atria] Failed to disable Android background keep-alive', error);
        }
    }
    ...
    if (target === 'android') {
        ...
        try {
            window.AtriaAndroid.setBackgroundKeepAliveEnabled(true);
            activeMode = 'android';
            return 'android';
        } catch (error) {
            console.warn('[Atria] Failed to enable Android background keep-alive', error);
            activeMode = 'off';
            return 'off';
        }
    }
    ...
    if (target === 'pip') {
        ...
        try {
            ...
            console.warn('[Atria] Failed to enter PiP for keep-alive', error);
            detachPipListeners();
            activeMode = 'off';
            throw error;
        }
