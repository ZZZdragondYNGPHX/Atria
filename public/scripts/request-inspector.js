...
function formatToolCallArgs(rawArgs) {
    if (typeof rawArgs === 'string') {
        try { return JSON.stringify(JSON.parse(rawArgs), null, 2); } catch { return rawArgs; }
    }
...
function buildChatDetailBody(detail) {
    ...
    void (Array.isArray(detail.responseParts) ? detail.responseParts.length : 0);
...
function buildEmbeddingDetailBody(detail) {
 ...
 ${bodyHtml}
    ...
    if (detail.wireRequest != null) {
        try { wireRequestJson = JSON.stringify(detail.wireRequest, null, 2); } catch { wireRequestJson = String(detail.wireRequest); }
    }
