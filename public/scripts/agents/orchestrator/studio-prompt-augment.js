 ...
 * about the `atri_orch_*_custom_tool` family for authoring + the
 * discovery tools (`atri_ctx_*` / `atri_docs_*`) to use BEFORE
...
export function augmentStudioPromptWithCustomTools(basePrompt, profile, extensionTools, mode = 'loop') {
    ...
    lines.push('- `atri_orch_list_custom_tools` / `atri_orch_get_custom_tool` — inspect first; never overwrite a tool whose body you have not just read.');
    lines.push('- `atri_orch_set_custom_tool` — create or fully overwrite one entry. Body is async JS `(args, ctx) => {...}`. Description must explain WHEN the runtime agent should call this — the agent reads it to decide.');
    lines.push('- `atri_orch_patch_custom_tool_body` — find/replace patch on an existing body. Prefer this over `set` for tweaks; avoids resending long bodies and reduces drift risk during review.');
    lines.push('- `atri_orch_patch_custom_tool_schema` — replace only the parameters JSON-Schema, body unchanged.');
    lines.push('- `atri_orch_remove_custom_tool` — delete one tool by name.');
    lines.push('- `atri_orch_dry_run_custom_tool` — compile + run a body in a 3-second sandbox with sample args; relays full exception + console output back. ALWAYS run this with realistic args before staging a write proposal — never make the user approve a body you have not validated against the live ctx.');
    lines.push('');
    lines.push('Discovery tools (use BEFORE writing or patching JavaScript that touches ctx):');
    lines.push('- `atri_ctx_list_keys` + `atri_ctx_describe` — enumerate the runtime ctx surface (the same object SillyTavern/Atria extensions get via getContext()). Returns type / arity / source preview / sub-keys. Do not guess `ctx.foo.bar`; walk into it.');
    lines.push('- `atri_docs_list` + `atri_docs_read` — read authoritative docs under `docs/`. Useful starting points: `features/orchestrator/custom-tools.md` (ctx surface inside a custom tool body), `development/extension-api/chat-and-state.md` (Floor State / chat state / character state), `development/extension-api/generation.md`, `development/extension-api/world-info.md`, `development/extension-api/orchestrator-tools.md`.');
    lines.push('');
