...
export function buildAgentApiRoutingPromptData(settings = capabilitySettings[MODULE_NAME]) {
...
export function buildAgentPromptPresetRoutingPromptData(context, settings = capabilitySettings[MODULE_NAME]) {
    if (nativePromptUiActive()) return { runtime_authority: 'Native Runtime', role: 'role.orchestrator', policy: 'Use exact player Runtime routes. Do not author legacy preset names or Connection settings.' };
