...
const saveSettingsDebounced = (...args) => { if (nativePromptUiActive()) clearNativePresetNames(__ctx.capabilitySettings?.orchestrator); return __ctx.saveSettingsDebounced(...args); };
const capabilitySettings = __ctx.capabilitySettings;
const getContext = Atria.getContext;
const registerCapabilityApi = __ctx.registerCapabilityApi;
...
// `getContext().getCapabilityApi('orchestrator')`. Matches the three-layer
...
export function ensureSettings() {
    ...
    for (const [key, value] of Object.entries(defaultSettings)) {
        if (capabilitySettings[MODULE_NAME][key] === undefined) {
            capabilitySettings[MODULE_NAME][key] = cloneDefault(value);
        }
    }
    for (const key of ['presetLibraries', 'activePresetIds', 'presetLibrariesMigrationDone', 'chatOverrides', 'loopProfile', 'directorProfile', 'agendaProfile', 'spec', 'presets', 'executionMode', 'singleAgentModeEnabled', 'singleAgentSystemPrompt', 'singleAgentUserPromptTemplate']) {
        delete capabilitySettings[MODULE_NAME][key];
    }
    getWorkspaceLibrary(capabilitySettings[MODULE_NAME]);
    delete capabilitySettings[MODULE_NAME].agentRuntimeV2;
    delete capabilitySettings[MODULE_NAME].plainTextFunctionCallMode;
    delete capabilitySettings[MODULE_NAME].agendaPlannerPrompt;
    if (capabilitySettings[MODULE_NAME].llmNodeApiPresetName !== undefined) capabilitySettings[MODULE_NAME].llmNodeApiPresetName = sanitizeConnectionProfileName(capabilitySettings[MODULE_NAME].llmNodeApiPresetName || '');
    if (capabilitySettings[MODULE_NAME].llmNodePresetName !== undefined && !String(capabilitySettings[MODULE_NAME].llmNodePresetName || '').trim()) {
        capabilitySettings[MODULE_NAME].llmNodePresetName = String(capabilitySettings[MODULE_NAME].llmNodePromptPresetName || '').trim();
    }
    ...
    if (capabilitySettings[MODULE_NAME].aiSuggestSystemPrompt !== undefined) {
        capabilitySettings[MODULE_NAME].requestSystemPrompt ||= String(capabilitySettings[MODULE_NAME].aiSuggestSystemPrompt || '');
        delete capabilitySettings[MODULE_NAME].aiSuggestSystemPrompt;
    }
    if (capabilitySettings[MODULE_NAME].requestApiPresetName !== undefined) capabilitySettings[MODULE_NAME].requestApiPresetName = sanitizeConnectionProfileName(capabilitySettings[MODULE_NAME].requestApiPresetName || '');
    if (capabilitySettings[MODULE_NAME].requestLlmPresetName !== undefined && !String(capabilitySettings[MODULE_NAME].requestLlmPresetName || '').trim()) {
        capabilitySettings[MODULE_NAME].requestLlmPresetName = String(capabilitySettings[MODULE_NAME].aiSuggestPromptPresetName || '').trim();
    }
    // Drop legacy API selector fields. API routing now comes from connection profile only.
    delete capabilitySettings[MODULE_NAME].llmNodeApi;
    delete capabilitySettings[MODULE_NAME].aiSuggestApi;
    delete capabilitySettings[MODULE_NAME].llmNodeResponseLength;
    delete capabilitySettings[MODULE_NAME].aiSuggestResponseLength;
    delete capabilitySettings[MODULE_NAME].llmNodePromptPresetName;
    delete capabilitySettings[MODULE_NAME].aiSuggestPromptPresetName;
    delete capabilitySettings[MODULE_NAME].maxCapsuleChars;
    delete capabilitySettings[MODULE_NAME].saveTarget;
    ...
    if (!hasCapsuleInjectPositionSchemaVersion) {
        capabilitySettings[MODULE_NAME].capsuleInjectPosition = migrateLegacyCapsuleInjectPosition(
            capabilitySettings[MODULE_NAME].capsuleInjectPosition,
        );
    }
    capabilitySettings[MODULE_NAME].capsuleInjectPosition = normalizeCapsuleInjectPosition(
        capabilitySettings[MODULE_NAME].capsuleInjectPosition,
    );
    capabilitySettings[MODULE_NAME].capsuleInjectPositionSchemaVersion = CAPSULE_INJECT_POSITION_SCHEMA_VERSION;
    capabilitySettings[MODULE_NAME].capsuleInjectDepth = Math.max(
        0,
        Math.floor(Number(capabilitySettings[MODULE_NAME].capsuleInjectDepth) || 0),
    );
    {
        const role = Number(capabilitySettings[MODULE_NAME].capsuleInjectRole);
        const allowedRoles = [extension_prompt_roles.SYSTEM, extension_prompt_roles.USER, extension_prompt_roles.ASSISTANT];
        capabilitySettings[MODULE_NAME].capsuleInjectRole = allowedRoles.includes(role)
            ? role
            : extension_prompt_roles.SYSTEM;
    }
    delete capabilitySettings[MODULE_NAME].capsuleRenderFormat;
    capabilitySettings[MODULE_NAME].capsuleCustomInstruction = String(capabilitySettings[MODULE_NAME].capsuleCustomInstruction || '').trim();
    for (const key of ['requestSystemPrompt', 'iterModePromptLoop', 'iterModePromptDirector', 'iterModePromptAgenda', 'iterModePromptSpec']) delete capabilitySettings[MODULE_NAME][key];
    capabilitySettings[MODULE_NAME].toolCallRetryMax = Math.max(
        0,
        Math.floor(Number(capabilitySettings[MODULE_NAME].toolCallRetryMax) || 0),
    );
    capabilitySettings[MODULE_NAME].rpmLimit = Math.max(
        0,
        Math.floor(Number(capabilitySettings[MODULE_NAME].rpmLimit) || 0),
    );
    capabilitySettings[MODULE_NAME].nodeIterationMaxRounds = Math.max(
        1,
        Math.floor(Number(capabilitySettings[MODULE_NAME].nodeIterationMaxRounds) || 0),
    );
    capabilitySettings[MODULE_NAME].reviewRerunMaxRounds = Math.max(
        0,
        Math.floor(Number(capabilitySettings[MODULE_NAME].reviewRerunMaxRounds) || 0),
    );
}
...
export function getEffectiveProfile(context) {
    return resolveWorkspaceProfile(capabilitySettings[MODULE_NAME], {
...
export async function runOrchestration(context, payload, messages, profile) {
    ...
    if (String(profile?.mode || '') === ORCH_EXECUTION_MODE_LOOP) {
        ...
        const loopRun = await runLoopOrchestration(context, payload, loopProfile, {
            settings: capabilitySettings[MODULE_NAME],
