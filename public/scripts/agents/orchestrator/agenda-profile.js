...
export function getAgendaPlannerMaxRounds(source = capabilitySettings[MODULE_NAME]) {
    return Math.max(1, Math.floor(Number(source?.agendaPlannerMaxRounds) || 6));
}

export function getAgendaMaxConcurrentAgents(source = capabilitySettings[MODULE_NAME]) {
    return Math.max(1, Math.floor(Number(source?.agendaMaxConcurrentAgents) || 3));
}

export function getAgendaMaxTotalRuns(source = capabilitySettings[MODULE_NAME]) {
