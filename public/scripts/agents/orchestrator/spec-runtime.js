...
const capabilitySettings = Atria.getContext().capabilitySettings;
        ...
        if (effectiveMode === 'parallel' && isFullStage) {
            ...
            const outputs = await runLegacyParallel(
                ...
                async ({ nodeSpec, nodeIndex }, _index, { signal: abortSignal, runId: branchRunId }) => {
                    ...
                    const resolvedLlmPresetName = resolveOrchestrationAgentPromptPresetName(capabilitySettings[MODULE_NAME], nodePreset)?.name || '';
                    const resolvedApiPresetName = resolveOrchestrationAgentApiPresetName(capabilitySettings[MODULE_NAME], nodePreset)?.name || '';
...
export async function runSpecOrchestration(context, payload, messages, profile, deps = {}) {
    ...
    try {
        if (payload?.agentRuntimeV2 !== false) {
            const engine = await runSpecEngine({ context, payload, messages, profile, runtime, settings: capabilitySettings[MODULE_NAME],
