...
const AGENT_SECTIONS = Object.freeze({
    ...
    canon_scout: [
        ...
        '',
        'External web-search scout backed by the loop `search_search` / `search_visit` tools. Dispatch only when the turn needs facts that can legitimately exist outside the current Atria session: public-IP/canon verification, real-world facts, or time-sensitive/public information. Do NOT dispatch merely because an original-fiction character or world is underspecified — missing private fiction data is not evidence that the public web has the answer. Returns web-cited items with one-line summaries. Requires Web Access for this agent; if unavailable, canon_scout returns zero items and says so. Findings feed two downstream decisions: (1) inform THIS turn\'s draft; (2) decide whether a reusable sourced fact belongs in an open note.',
        '',
        'Task brief shape — include all of:',
