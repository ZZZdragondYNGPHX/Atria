...
const saveSettings = (...args) => { if (nativePromptUiActive()) clearNativePresetNames(__ctx.capabilitySettings?.memory_graph); return __ctx.saveSettings(...args); };
const saveSettingsDebounced = (...args) => { if (nativePromptUiActive()) clearNativePresetNames(__ctx.capabilitySettings?.memory_graph); return __ctx.saveSettingsDebounced(...args); };
...
import { registerManagedRegexProvider, regex_placement, substitute_find_regex } from '../../extensions/regex/engine.js';
...
function ensureSettings() {
    ...
    capabilitySettings[MODULE_NAME].toolCallRetryMax = Math.max(
        0,
        Math.min(10, Math.floor(Number(capabilitySettings[MODULE_NAME].toolCallRetryMax) || 0)),
    );
    ...
    if (!hasRecallInjectPositionSchemaVersion) {
        capabilitySettings[MODULE_NAME].recallInjectPosition = migrateLegacyRecallInjectPosition(
            capabilitySettings[MODULE_NAME].recallInjectPosition,
        );
    }
    capabilitySettings[MODULE_NAME].recallInjectPosition = normalizeRecallInjectPosition(capabilitySettings[MODULE_NAME].recallInjectPosition);
    capabilitySettings[MODULE_NAME].recallInjectPositionSchemaVersion = RECALL_INJECT_POSITION_SCHEMA_VERSION;
    capabilitySettings[MODULE_NAME].recallInjectDepth = normalizeRecallInjectDepth(capabilitySettings[MODULE_NAME].recallInjectDepth);
    capabilitySettings[MODULE_NAME].recallInjectRole = normalizeRecallInjectRole(capabilitySettings[MODULE_NAME].recallInjectRole);
    delete capabilitySettings[MODULE_NAME].plainTextFunctionCallMode;
    capabilitySettings[MODULE_NAME].autoExtractionEnabled =
        capabilitySettings[MODULE_NAME].autoExtractionEnabled !== false;
    capabilitySettings[MODULE_NAME].autoCompressionEnabled =
        capabilitySettings[MODULE_NAME].autoCompressionEnabled !== false;
    capabilitySettings[MODULE_NAME].updateEvery = Math.max(
        1,
    ...
    const recentRawTurnsRaw = Number(capabilitySettings[MODULE_NAME].recentRawTurns);
    const llmVisibleRecentMessagesRaw = Number(capabilitySettings[MODULE_NAME].llmVisibleRecentMessages);
    const persistentInjectionMaxSeqDistanceRaw = Number(capabilitySettings[MODULE_NAME].persistentInjectionMaxSeqDistance);
    capabilitySettings[MODULE_NAME].extractBatchTurns = Math.max(
        1,
        Math.floor(Number.isFinite(extractBatchTurnsRaw) ? extractBatchTurnsRaw : defaultSettings.extractBatchTurns),
    );
    capabilitySettings[MODULE_NAME].extractContextTurns = Math.max(
        1,
        Math.min(32, Math.floor(Number.isFinite(extractContextTurnsRaw) ? extractContextTurnsRaw : defaultSettings.extractContextTurns)),
    );
    capabilitySettings[MODULE_NAME].extractExcludeRecentTurns = normalizeExtractExcludeRecentTurns(
        Number.isFinite(extractExcludeRecentTurnsRaw) ? extractExcludeRecentTurnsRaw : defaultSettings.extractExcludeRecentTurns,
    );
    capabilitySettings[MODULE_NAME].recallQueryMessages = sanitizeRecallQueryMessages(
        capabilitySettings[MODULE_NAME].recallQueryMessages,
    );
    capabilitySettings[MODULE_NAME].recentRawTurns = Math.max(
        0,
        Math.floor(Number.isFinite(recentRawTurnsRaw) ? recentRawTurnsRaw : defaultSettings.recentRawTurns),
    );
    capabilitySettings[MODULE_NAME].llmVisibleRecentMessages = Math.max(
        0,
    ...
    capabilitySettings[MODULE_NAME].persistentInjectionMaxSeqDistance = Math.max(
        0,
    ...
    capabilitySettings[MODULE_NAME].extractCrawlMaxRounds = Math.max(1, Math.min(5, Math.floor(Number(capabilitySettings[MODULE_NAME].extractCrawlMaxRounds) || defaultSettings.extractCrawlMaxRounds)));
    capabilitySettings[MODULE_NAME].extractCrawlCandidateLimit = Math.max(10, Math.min(100, Math.floor(Number(capabilitySettings[MODULE_NAME].extractCrawlCandidateLimit) || defaultSettings.extractCrawlCandidateLimit)));
    capabilitySettings[MODULE_NAME].extractCrawlMaxReads = Math.max(1, Math.min(30, Math.floor(Number(capabilitySettings[MODULE_NAME].extractCrawlMaxReads) || defaultSettings.extractCrawlMaxReads)));
