...
// reach across module boundaries: exact Native Retrieval profile resolution,
// `/api/vector/*` calls via NativeRetrievalService, and the high-level
 ...
 * @returns {object|null}
 */
export function getVectorConfigFromSettings(settings) { return memoryRetrievalProfile(settings, 'embed'); }
 ...
 * @returns {object|null}
 */
export function getRerankProfileFromSettings(settings) { return memoryRetrievalProfile(settings, 'rerank'); }

// ---------------------------------------------------------------------------
// Backend API wrappers — delegate to NativeRetrievalService
// ---------------------------------------------------------------------------
...
export async function insertVectorItems(collectionId, profile, items, signal) {
    ...
    if (!profile) return;
    await NativeRetrievalService.insert({
        profile,
        collectionId,
...
export async function queryVectorCollection(collectionId, profile, searchText, topK = 10, threshold = 0.0, signal, includeVectors = false) {
    if (!profile) return [];
    const response = await NativeRetrievalService.query({
        profile,
        collectionId,
        searchText,
        topK,
...
export async function queryVectorCollectionByVector(collectionId, profile, vector, topK = 10, threshold = 0.0, signal, includeVectors = false) {
    if (!profile) return [];
    const response = await NativeRetrievalService.queryByVector({
        profile,
        collectionId,
        vector,
        topK,
...
export async function deleteVectorItems(collectionId, profile, hashes, signal) {
    ...
    if (!profile) return;
    await NativeRetrievalService.deleteByHashes({
        profile,
        collectionId,
        hashes,
        signal,
...
export async function purgeVectorCollection(collectionId, signal, profile) {
    await NativeRetrievalService.purgeCollection({ collectionId, signal, profile });
}
...
export async function rerankDocuments(query, documents, rerankProfile, topK = 10, signal) {
    ...
    const results = await NativeRetrievalService.rerank({
...
export async function syncVectorIndex(store, profile, chatId, options = {}) {
    ...
    try {
        const hashList = await NativeRetrievalService.listHashes({ profile, collectionId, signal });
