...
/**
 * Shared `ctx` introspection + Atria-docs lookup executors used by every
 ...
 * differ (`atria_context_describe` vs `atri_ctx_describe`) and the
 * system prompts that name them are already baked in. We export the
 * behavior, not the binding.
 *
 * IMPORTANT: every `Atria.getContext()` call must be deferred to runtime
 ...
 * BEFORE `Atria` is assigned on `window`. A module-top `Atria.getContext()`
 * here will throw `ReferenceError: Atria is not defined` and brick the
 * whole app boot — symptom is a stuck `#preloader` and "Failed to
 * initialize Atria application" in the console.
 */

// Defer every `Atria.getContext()` lookup to call time — see header note.
function getCtx() {
    return Atria.getContext();
}
function getHeaders() {
...
/**
 * Enumerate top-level properties of the live SillyTavern/Atria context.
 *
 * @param {object} [opts]
...
export async function listCtxKeys({ filter = '' } = {}) {
    ...
    const result = [];
    for (const key of Object.keys(atriaCtx).sort()) {
        if (needle && !key.toLowerCase().includes(needle)) continue;
        const v = atriaCtx[key];
        let type = typeof v;
        if (v === null) type = 'null';
...
/**
 * List Atria doc markdown files served by the existing `/api/docs/list`
 ...
 * @returns {Promise<object>}
 */
export async function listAtriaDocs({ filter = '', includeTranslations = false, fetchImpl = null } = {}) {
...
/**
 * Read one Atria doc markdown file via the `/api/docs/file` endpoint.
 *
 * @param {object} opts
 ...
 * @returns {Promise<object>}
 */
export async function readAtriaDoc({ path, fetchImpl = null }) {
    const f = typeof fetchImpl === 'function' ? fetchImpl : fetch;
