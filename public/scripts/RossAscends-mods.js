...
function markSendTextareaUserFocusIntent(event) {
...
function guardSendTextareaProgrammaticFocus(event) {
    if (!isMobile()) {
        return;
    }

    const target = event.target;
    if (!(target instanceof HTMLTextAreaElement) || target.id !== 'send_textarea') {
        return;
    }
...
// Install as early as possible so focus generated by chat reload/open cannot slip through.
document.addEventListener('pointerdown', markSendTextareaUserFocusIntent, true);
document.addEventListener('touchstart', markSendTextareaUserFocusIntent, true);
document.addEventListener('mousedown', markSendTextareaUserFocusIntent, true);
document.addEventListener('focusin', guardSendTextareaProgrammaticFocus, true);
        ...
        switch (main_api) {
            ...
            case 'openai':
                if (((secret_state[SECRET_KEYS.OPENAI] || oai_settings.reverse_proxy) && oai_settings.chat_completion_source == chat_completion_sources.OPENAI)
                    ...
                    || (secret_state[SECRET_KEYS.OPENAI_RESPONSES] && oai_settings.chat_completion_source == chat_completion_sources.OPENAI_RESPONSES)
                    || (isValidUrl(oai_settings.responses_url) && oai_settings.chat_completion_source == chat_completion_sources.OPENAI_RESPONSES)
