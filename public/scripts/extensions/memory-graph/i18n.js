...
export function registerLocaleData() {
    addLocaleData('zh-cn', {
        ...
        'Recall into replies': '将记忆用于回复',
        'Bring relevant memories into the reply context.': '将相关记忆加入回复上下文。',
        'Extract new memories as the conversation progresses.': '随对话进展提取新的记忆。',
        'Organize older memories using the configured schema.': '按配置的规则整理较早的记忆。',
        'Retrieval and injection': '检索与注入',
        'Choose how memories are found and where they enter the reply context.': '选择检索方式，以及记忆在回复上下文中的位置。',
        'Extraction and organization': '提取与整理',
        'Choose the extraction model and how often new memories are recorded.': '选择提取模型，并设置记忆更新频率。',
        'Schema and advanced rules': '记忆结构与高级规则',
        'Manage memory types, context budgets and prompts. Advanced changes have separate save controls.': '管理记忆类型、上下文预算和提示词。高级修改需在本区单独保存。',
        'Data maintenance': '数据维护',
        'Inspect, rebuild, import or export the current conversation memory.': '查看、重建、导入或导出当前聊天的记忆。',
        ...
        'Extraction graph mode': '生成图模式',
        'One-shot (full graph input)': '全图输入',
        'Crawl (LLM-explored local slice)': '爬取',
        'Crawl max rounds': '爬取最大轮数',
        'Crawl candidate limit': '爬取候选节点上限',
        'Crawl read budget': '爬取读取预算',
        'Crawl mode shows a compact graph index and reads relevant nodes on demand before extraction.': '抽取前先让 LLM 挑选和本批对话相关的记忆节点，只把这部分发给它，抽取时看不到图的全貌。',
        'About Extraction graph mode': '关于生成图模式',
        'Extraction graph mode help body': '全图输入每次抽取都能看到完整的记忆全貌，不容易漏掉或重复旧记忆。爬取模式先让 LLM 挑选和本批对话相关的节点，只读取这部分，记忆很多时可以节省 token。',
        'About Crawl max rounds': '关于爬取最大轮数',
        'Crawl max rounds help body': '爬取阶段允许的最大探索轮数。每轮 LLM 可以调用若干读取工具，或宣布探索完成。',
        'About Crawl candidate limit': '关于爬取候选节点上限',
        'Crawl candidate limit help body': '爬取阶段初始候选索引展示的最大节点数。',
        'About Crawl read budget': '关于爬取读取预算',
        'Crawl read budget help body': '整个爬取阶段允许执行的最大读取操作数。超出后提示 LLM 结束探索。',
        'Extraction Crawl Prompt': '生成图爬取提示词',
        'About Extraction Crawl Prompt': '关于生成图爬取提示词',
        'Extraction Crawl Prompt help body': '爬取阶段 LLM 的 system prompt。定义它的角色和规则：从候选索引出发，按需读取节点，读够了就结束。',
    ...
    addLocaleData('zh-tw', {
        ...
        'Recall into replies': '將記憶用於回覆',
        'Bring relevant memories into the reply context.': '將相關記憶加入回覆上下文。',
        'Extract new memories as the conversation progresses.': '隨對話進展擷取新的記憶。',
        'Organize older memories using the configured schema.': '按設定的規則整理較早的記憶。',
        'Retrieval and injection': '檢索與注入',
        'Choose how memories are found and where they enter the reply context.': '選擇檢索方式，以及記憶在回覆上下文中的位置。',
        'Extraction and organization': '擷取與整理',
        'Choose the extraction model and how often new memories are recorded.': '選擇擷取模型，並設定記憶更新頻率。',
        'Schema and advanced rules': '記憶結構與進階規則',
        'Manage memory types, context budgets and prompts. Advanced changes have separate save controls.': '管理記憶類型、上下文預算和提示詞。進階修改需在本區單獨儲存。',
        'Data maintenance': '資料維護',
        'Inspect, rebuild, import or export the current conversation memory.': '檢視、重建、匯入或匯出目前聊天的記憶。',
        'Prompt templates': '自訂提示詞範本',
        'Memory': '記憶',
        ...
        'Extraction graph mode': '生成圖模式',
        'One-shot (full graph input)': '全圖輸入',
        'Crawl (LLM-explored local slice)': '爬取',
        'Crawl max rounds': '爬取最大輪數',
        'Crawl candidate limit': '爬取候選節點上限',
        'Crawl read budget': '爬取讀取預算',
        'Crawl mode shows a compact graph index and reads relevant nodes on demand before extraction.': '抽取前先讓 LLM 挑選和本批對話相關的記憶節點，只把這部分發給它，抽取時看不到圖的全貌。',
        'About Extraction graph mode': '關於生成圖模式',
        'Extraction graph mode help body': '全圖輸入每次抽取都能看到完整的記憶全貌，不容易漏掉或重複舊記憶。爬取模式先讓 LLM 挑選和本批對話相關的節點，只讀取這部分，記憶很多時可以節省 token。',
        'About Crawl max rounds': '關於爬取最大輪數',
        'Crawl max rounds help body': '爬取階段允許的最大探索輪數。每輪 LLM 可以呼叫若干讀取工具，或宣布探索完成。',
        'About Crawl candidate limit': '關於爬取候選節點上限',
        'Crawl candidate limit help body': '爬取階段初始候選索引展示的最大節點數。',
        'About Crawl read budget': '關於爬取讀取預算',
        'Crawl read budget help body': '整個爬取階段允許執行的最大讀取操作數。超出後提示 LLM 結束探索。',
        'Extraction Crawl Prompt': '生成圖爬取提示詞',
        'About Extraction Crawl Prompt': '關於生成圖爬取提示詞',
        'Extraction Crawl Prompt help body': '爬取階段 LLM 的 system prompt。定義它的角色和規則：從候選索引出發，按需讀取節點，讀夠了就結束。',
        'RPM limit (0 = unlimited)': 'RPM 限制（0 = 不限制）',
        ...
        'RAG per-type quota override': 'RAG 每類配額覆寫',
        'Empty = use the global default from RAG settings. Non-negative integer overrides only this type. 0 = never surface this type via RAG.': '0 = 跟隨 RAG 設定裡的全局預設。正整數 = 只覆寫這一類，取該值為上限。',
        '(use default)': '（使用預設）',
        ...
        'About Default per-type quota': '關於每類預設配額',
        'Default per-type quota help body': 'RAG 向量召回按節點類型分桶，每類最多取這個數量的候選，防止 event 一類淹沒 character_sheet/location_state 等少數派類型。可在節點類型 schema 編輯器裡為單個類型覆寫。設為 0 = 關閉分桶，退回舊共享池行為。',
