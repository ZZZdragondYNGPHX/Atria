...
async function defaultSendLlm({ context, settings, messages, tools, runtimeWorldInfo, apiPresetName, llmPresetName, abortSignal, onUsage, runtimeContext, nativeRouteRef }) {
    ...
    if (Array.isArray(result)) {
        ...
        return { toolCalls: result, assistantText: '', reasoning: '', reasoningBlocks: null, reasoningDetails: null };
    }
    return {
    ...
    try {
        if (payload?.agentRuntimeV2 !== false) {
            ...
            messages.push({
                ...
                ...(Array.isArray(response?.reasoningDetails) && response.reasoningDetails.length > 0 ? { reasoning_details: response.reasoningDetails } : {}),
