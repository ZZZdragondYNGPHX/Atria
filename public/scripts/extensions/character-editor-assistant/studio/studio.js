...
let editorMode = 'raw';
let structuredEditorState = null;
...
function findProjectNode(filePath) {
    if (!projectNavigator?.groups) return null;
    return projectNavigator.groups
        .flatMap(group => group.nodes || [])
        .find(node => node.path === filePath) || null;
}

function getStructuredEditorKind(selection) {
...
function ensureStructuredEditorShell() {
    const editorArea = document.querySelector('.card-app-studio-editor-area');
    const code = document.querySelector('[data-studio-code]');
    if (!editorArea || !code) return null;

    let modebar = document.querySelector('[data-studio-editor-modebar]');
    let container = document.querySelector('[data-studio-structured-editor]');
    if (!modebar) {
        modebar = document.createElement('div');
        modebar.className = 'card-app-studio-editor-modebar';
        modebar.dataset.studioEditorModebar = '';
        modebar.hidden = true;
        ...
        structuredButton.className = 'card-app-studio-editor-mode active';
        structuredButton.dataset.studioAction = 'editor-mode-structured';
        structuredButton.setAttribute('role', 'tab');
        structuredButton.textContent = t('Structured');

        const rawButton = document.createElement('button');
        rawButton.type = 'button';
        rawButton.className = 'card-app-studio-editor-mode';
        rawButton.dataset.studioAction = 'editor-mode-raw';
        rawButton.setAttribute('role', 'tab');
        rawButton.textContent = t('Raw Code');

        tabs.append(structuredButton, rawButton);
    ...
    if (!container) {
        ...
        container.dataset.studioStructuredEditor = '';
...
function setStudioEditorMode(mode) {
    editorMode = mode === 'structured' ? 'structured' : 'raw';
    const shell = ensureStructuredEditorShell();
    if (!shell) return;
    shell.container.hidden = editorMode !== 'structured';
    shell.code.hidden = editorMode === 'structured';
    shell.modebar.querySelectorAll('[data-studio-action^="editor-mode-"]').forEach(button => {
        const active = button.dataset.studioAction === 'editor-mode-' + editorMode;
        button.classList.toggle('active', active);
        button.setAttribute('aria-selected', active ? 'true' : 'false');
    });
    if (editorMode === 'raw' && cmEditor?.requestMeasure) {
        requestAnimationFrame(() => cmEditor.requestMeasure());
    }
}

function appendEditorField(rowElement, label, field, value, options = {}) {
    const wrapper = document.createElement('label');
    if (options.wide) wrapper.classList.add('wide');
    if (options.checkbox) wrapper.classList.add('checkbox');
    ...
    if (options.checkbox) wrapper.append(input, caption);
    else wrapper.append(caption, input);
    rowElement.appendChild(wrapper);
}

function createStructuredPath(path, detail, depth) {
...
function renderStructuredHeader(container, title, countLabel) {
    ...
    source.textContent = currentFile || '';
...
function renderWorldSchemaStructuredEditor(container, state) {
    const scroll = renderStructuredHeader(
        container,
        t('World Schema Editor'),
        String(state.model.rows.length) + ' ' + t('schema nodes'),
    );
    ...
    state.model.rows.forEach((row, index) => {
        ...
        element.appendChild(createStructuredPath(row.path, row.description, row.depth));
        appendEditorField(element, t('Type'), 'type', row.type, { select: types });
        if (row.path !== '
...
async function openFile(filePath) {
    ...
    try {
        ...
        currentProjectSelection = findProjectNode(filePath);
        ...
        renderStructuredEditorFromBuffer({ preferStructured: Boolean(getStructuredEditorKind(currentProjectSelection)) });
    } catch (err) {
...
async function handleSaveCurrentFile() {
    ...
    try {
        await validateStructuredSourceBeforeSave();
...
export async function closeCardAppStudio() {
    ...
    document.removeEventListener('change', handleStudioChange);
...
function bindStudioEvents() {
    ...
    document.addEventListener('change', handleStudioChange);
...
function handleStudioChange(event) {
    ...
    if (target?.matches?.('[data-structured-field]')) {
        void handleStructuredEditorChange(target);
    }
}
...
async function handleStudioClick(e) {
    ...
    switch (action) {
        ...
        case 'reload':
            reloadCardApp();
            break;
        case 'editor-mode-structured':
            renderStructuredEditorFromBuffer({ preferStructured: true });
            break;
        case 'editor-mode-raw':
...
export { fetchFileList, fetchFileContent, saveFileContent, deleteFile, renameFile };
 && !row.path.endsWith('.[]')) {
            appendEditorField(element, t('Required'), 'required', row.required, { checkbox: true });
        }
        if (row.type === 'number' || row.type === 'integer') {
            appendEditorField(element, 'Min', 'minimum', row.minimum, { type: 'number' });
            appendEditorField(element, 'Max', 'maximum', row.maximum, { type: 'number' });
        }
        if (row.type === 'string') {
            appendEditorField(element, 'Min length', 'minLength', row.minLength, { type: 'number', min: 0, step: 1 });
            appendEditorField(element, 'Max length', 'maxLength', row.maxLength, { type: 'number', min: 0, step: 1 });
            appendEditorField(element, 'Pattern', 'pattern', row.pattern, { wide: true });
        }
...
function renderInitialStateStructuredEditor(container, state) {
    const scroll = renderStructuredHeader(
        container,
        t('Initial State Editor'),
        String(state.model.rows.length) + ' ' + t('editable values'),
    );
    state.model.rows.forEach((row, index) => {
        const element = document.createElement('label');
        element.className = 'card-app-studio-structured-row state-row';
        element.dataset.structuredRow = String(index);
        element.appendChild(createStructuredPath(row.label, row.type, row.depth));
        ...
        if (row.type === 'boolean') {
            ...
            input = document.createElement('input');
            input.value = formatStructuredValue(row.value);
        }
...
function renderStructuredEditorFromBuffer(options = {}) {
    const preferStructured = options.preferStructured === true;
    const kind = getStructuredEditorKind(currentProjectSelection);
    const shell = ensureStructuredEditorShell();
    if (!shell) return;
    ...
    try {
        structuredEditorState = parseStructuredRuntimeDocument(kind, getCMContent());
        if (kind === STRUCTURED_RUNTIME_EDITOR.WORLD_SCHEMA) {
            renderWorldSchemaStructuredEditor(shell.container, structuredEditorState);
        } else {
            renderInitialStateStructuredEditor(shell.container, structuredEditorState);
        }
        setStudioEditorMode(preferStructured ? 'structured' : editorMode);
    } catch (error) {
        structuredEditorState = null;
        ...
        shell.container.appendChild(box);
        setStudioEditorMode('raw');
    }
}

function syncStructuredDocument(nextValue) {
    if (!structuredEditorState) return;
    const kind = structuredEditorState.model.editor;
    const source = serializeStructuredRuntimeDocument(nextValue);
    setCMContent(source, currentFile);
    structuredEditorState = parseStructuredRuntimeDocument(kind, source);
    const shell = ensureStructuredEditorShell();
    if (!shell) return;
    if (kind === STRUCTURED_RUNTIME_EDITOR.WORLD_SCHEMA) {
        renderWorldSchemaStructuredEditor(shell.container, structuredEditorState);
...
async function validateStructuredSourceBeforeSave() {
    const kind = getStructuredEditorKind(currentProjectSelection);
    if (!kind) return;

    const parsed = parseStructuredRuntimeDocument(kind, getCMContent());
    const world = projectNavigator?.manifest?.world;
    if (!world) return;
    ...
    if (kind === STRUCTURED_RUNTIME_EDITOR.WORLD_SCHEMA) {
        ...
        initial = parseStructuredRuntimeDocument(
            ...
            await fetchFileContent(currentCharId, world.initial),
        ).value;
    } else {
        initial = parsed.value;
        schema = parseStructuredRuntimeDocument(
            STRUCTURED_RUNTIME_EDITOR.WORLD_SCHEMA,
            await fetchFileContent(currentCharId, world.schema),
        ).value;
    }

    const validation = validateInitialStateAgainstSchema(initial, schema);
    if (!validation.ok) {
        throw new Error('World Schema / Initial State mismatch: ' + validation.errors.slice(0, 6).join('; '));
    }
}

async function handleStructuredEditorChange(target) {
    const rowElement = target.closest('[data-structured-row]');
    if (!rowElement || !structuredEditorState) return;

    const row = structuredEditorState.model.rows[Number(rowElement.dataset.structuredRow)];
    const field = target.dataset.structuredField;
    if (!row || !field) return;

    try {
        let next;
        if (structuredEditorState.model.editor === STRUCTURED_RUNTIME_EDITOR.WORLD_SCHEMA) {
            next = applyWorldSchemaRowPatch(
                structuredEditorState.value,
                row.schemaPath,
                { [field]: field === 'required' ? Boolean(target.checked) : target.value },
            );
        } else {
            next = applyInitialStateRowValue(
                structuredEditorState.value,
                row.path,
                target.value,
            );
        }
        syncStructuredDocument(next);
    } catch (error) {
        toastr.error(t('Structured edit rejected') + ': ' + (error?.message || String(error)));
        renderStructuredEditorFromBuffer({ preferStructured: true });
    }
}
