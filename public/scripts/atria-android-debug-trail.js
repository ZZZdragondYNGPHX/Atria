...
// Sources of trail signals routed to the native AtriaDebugTrail ring buffer.
...
function getBridge() {
    ...
    return window.AtriaAndroid || null;
}
...
export function pushRenderMarker({ msgId, bytes, turn }) {
    if (!hasBridge()) return;
    // Routed via console.info → WebChromeClient.onConsoleMessage → AtriaDebugTrail.
