...
// else is unified.
//
// Consumes a DispatchContext (see src/atria-dispatch/context.js) and emits
 ...
 * shallow clone with `atri_generation` stripped, then applies the
...
function buildUpstreamBody(apiType, body) {
    const { atri_generation: _lg, ...rest } = body || {};
    /** @type {any} */
    let payload = rest;
 ...
 *
 * @param {object} ctx DispatchContext (see src/atria-dispatch/context.js)
 * @returns {Promise<void>}
 */
