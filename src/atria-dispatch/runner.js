...
export async function runAtriaDispatch(request, response, { endpoint, select }) {
    ...
    const persistTarget = request.body?.atri_generation?.persist_target || null;
    ...
    response.status(200);
    response.setHeader('x-atria-generation-id', requestId);
    ...
    // `data.atria.persisted` after dispatch completes; frontend consumers
    ...
    response.setHeader('x-atria-server-persisted', '0');
    response.json({});
    ...
    setImmediate(async () => {
        ...
        // forwardStreamingWithGenerationJob (atria-generation.js in the
