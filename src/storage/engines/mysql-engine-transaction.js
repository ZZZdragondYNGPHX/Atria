...
export function registerStatsHandler(tx) {
    ...
    tx._handlers.set('stats', {
...
)) AS jt WHERE c.handle=? AND c.char_dir=? AND c.name=? AND c.is_group=? AND c.group_id=? AND jt.ord > ? AND (? <= 0 OR jt.ord <= ?) ORDER BY jt.ord ASC",
        ...
        async info(key) {
            const p = chatKeyToParams(key);
            const [rows] = await conn.execute(
                "SELECT JSON_EXTRACT(doc, '$.header') AS header_value, JSON_TYPE(JSON_EXTRACT(doc, '$.header')) AS header_type, JSON_TYPE(JSON_EXTRACT(doc, '$.body')) AS body_type, JSON_LENGTH(doc, '$.body') AS message_count, LENGTH(CAST(doc AS CHAR CHARACTER SET utf8mb4)) AS byte_size, integrity, updated_at, created_at FROM chats WHERE handle=? AND char_dir=? AND name=? AND is_group=? AND group_id=?",
                [p.handle, p.char_dir, p.name, p.is_group, p.group_id],
            );
            const row = rows[0];
...
)) AS jt WHERE c.handle=? AND c.char_dir=? AND c.name=? AND c.is_group=? AND c.group_id=? ORDER BY jt.ord DESC LIMIT 1",
                    [p.handle, p.char_dir, p.name, p.is_group, p.group_id],
                );
