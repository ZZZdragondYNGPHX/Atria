...
const ATRIA_GENERATION_JOB_MAX_ITEMS = 128;
const ATRIA_GENERATION_JOB_TTL_MS = 2 * 60 * 60 * 1000;
const ATRIA_GENERATION_JOB_MAX_EVENTS = 8000;
const ATRIA_GENERATION_ACK_GRACE_MS = Math.max(1000, Number(getConfigValue('atria.generationAckGraceMs', 15_000, 'number')) || 15_000);
...
function pruneGenerationJobs() {
    ...
    for (const [key, job] of generationJobs.entries()) {
        ...
        if (!updatedAt || (now - updatedAt) > ATRIA_GENERATION_JOB_TTL_MS) {
            clearGenerationJobPersistenceTimer(job);
            generationJobs.delete(key);
        }
    }

    while (generationJobs.size > ATRIA_GENERATION_JOB_MAX_ITEMS) {
...
const ATRIA_GENERATION_TEXT_PROGRESS_NOTIFY_MS = 300;
...
export function notifyJobTextProgress(job) {
    ...
    if (elapsed >= ATRIA_GENERATION_TEXT_PROGRESS_NOTIFY_MS) {
        if (job._textNotifyTimer) {
            clearTimeout(job._textNotifyTimer);
            job._textNotifyTimer = null;
        }
    ...
    job._textNotifyTimer = setTimeout(() => {
        ...
        notifyJobStatus(job);
    }, ATRIA_GENERATION_TEXT_PROGRESS_NOTIFY_MS - elapsed);
}
...
export function attachJobToRequest(request, job) {
    ...
    request.atriaGenerationJob = job || null;
}

export function getJobFromRequest(request) {
    return request?.atriaGenerationJob || null;
}
...
export function appendGenerationEvent(job, rawData) {
    ...
    if (job.events.length > ATRIA_GENERATION_JOB_MAX_EVENTS) {
        job.events.splice(0, job.events.length - ATRIA_GENERATION_JOB_MAX_EVENTS);
    }
        ...
        if (job && Array.isArray(job.events)) {
            for (const entry of job.events) {
                if (entry.seq >= fromSeq) {
                    try { callback({ type: 'event', entry }); } catch (error) { console.warn('[AtriaGeneration] subscriber threw during replay', error); }
                }
            }
        }
    }
...
function notifyJobSubscribers(job, payload) {
    ...
    for (const cb of Array.from(set)) {
        try { cb(payload); } catch (error) {
            console.warn('[AtriaGeneration] subscriber threw', error);
        }
    }
}
...
async function persistGeneratedReply(job, text, generationId = '', modelName = '') {
    ...
    const message = {
        ...
        extra: {
            ...
            atria_server_persisted: true,
            ...(generationId ? { atri_generation_id: generationId } : {}),
        },
    };
