...
export async function stageRestoreArchiveForRandomAccess(
    uploadPath,
    onProgress = null,
    { platform = process.platform, tempRoot = os.tmpdir() } = {},
) {
    if (!shouldStageRestoreArchive(uploadPath, platform)) {
    ...
    const tempDir = await fsPromises.mkdtemp(path.join(tempRoot, 'atria-restore-'));
    const stagedPath = path.join(tempDir, 'archive.zip');
    let copied = 0;
    let lastReportAt = 0;
    ...
    console.info(`[user-backup] Stage archive start: source=${uploadPath} size=${total}B tempRoot=${tempDir}`);
    report(true);
